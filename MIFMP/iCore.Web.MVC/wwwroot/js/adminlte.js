...
//输入组件change事件
function OnEditorValueChanged(compName, formId) {


    $("#" + formId).each(function () {

        for (var i = 0; i < this.elements.length; i++) {
            if (this.elements[i].getAttribute("cascade-field") == compName) {
                ClearEditorItems(this.elements[i]);
                GetEditorItems(this.elements[i], formId);
            }
        }
    });

}
function ClearEditorItems(element) {
    $(element).find("option").remove();
}

function GetEditorItems(element, formId) {

    var bindcommand = element.getAttribute("bind-command");
    var bindtype = element.getAttribute("bind-type");
    var tagname = element.tagName;
    var formdata = "BindCommand=" + bindcommand + "&";
    formdata += $("#" + formId).serialize();
    $.ajax({
        type: "POST",
        url: "/InternalAPI/BindData/GetData",
                    ...
                    case "select":
                        {
                            for (var i = 0; i < response.Data.length; i++) {
                                $(element).append('<option value="{0}">{1}</option>'.format(response.Data[i].Key, response.Data[i].Value))
                            }
                            break;
                        }
                }
